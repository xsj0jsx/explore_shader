...
GLuint programe_directionLight, show_normal_program, programe_pointLight, programe_spotLight;
...
void init()
{
	...
	//
	ShaderType_Fname normal_shaders = { {GL_VERTEX_SHADER, "normal_vertex.shader"}
							 ,{GL_GEOMETRY_SHADER, "geometry.shader"}
							 ,{GL_FRAGMENT_SHADER, "fragment.shader"}};
	show_normal_program = buildProgrameFromFile(normal_shaders);
	
	ShaderType_Fname face_shaders = {{GL_VERTEX_SHADER, "directionalLight_vertex.shader"}
									 ,{GL_FRAGMENT_SHADER, "fragment.shader"}};
	programe_directionLight = buildProgrameFromFile(face_shaders);
	
	ShaderType_Fname pointLight_shaders = {{GL_VERTEX_SHADER, "pointLight_vertex.shader"}
										   ,{GL_FRAGMENT_SHADER, "fragment.shader"}};
	programe_pointLight = buildProgrameFromFile(pointLight_shaders);
	ShaderType_Fname spotLight_shaders = {{GL_VERTEX_SHADER, "spotLight_vertex.shader"}
										  ,{GL_FRAGMENT_SHADER, "fragment.shader"}};
	programe_spotLight = buildProgrameFromFile(spotLight_shaders);
	//
	programe_current = programe_pointLight;
}
...
void keyboard(unsigned char key, int x, int y)
{
	...
	{
	case '1':
		programe_current = programe_directionLight;
		break;
	case '2':
		programe_current = programe_pointLight;
		break;
	case '3':
		programe_current = programe_spotLight;
		break;
	case 27:
		glutLeaveMainLoop();
		break;
